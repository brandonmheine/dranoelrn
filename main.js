...
const drinkCardContainer = document.querySelector(
  '[data-drink-cards-container]'
)
...
function getRecipe() {
  const selectedKeys = Object.keys(drinkVarietyObject).filter(
  ...
  const recipeElements = drinkIngredient.forEach((ingredient, index) => {
    ...
    lists.textContent = `${drinkMeasure[index] || ' '} ${
      drinkIngredient[index]
    }`
